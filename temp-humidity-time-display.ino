//This code shows temperature humidity and time in display
...
void setup() {
  ...
  // Setup initial display (keeping your original format)
  lcd.clear();
  lcd.print("Temp:    --.- ");
  lcd.setCursor(0, 1);
  lcd.print("Humidity:  -- %");
...
void updateTimeDisplay() {
  ...
  lcd.setCursor(0, 3);
  lcd.print(String(ptm->tm_hour) + ":" + 
           (ptm->tm_min < 10 ? "0" : "") + String(ptm->tm_min) + " " +
           monthAbbr[ptm->tm_mon] + " " + String(ptm->tm_mday) + "    ");
}
...
void loop() {
  ...
  // Read sensors (keeping your original code)
  float temp = sht31.readTemperature();
  float hum = sht31.readHumidity();

  // Update temperature display (original format)
  if (!isnan(temp)) {
    lcd.setCursor(7, 0);
    lcd.print(String(temp, 1) + " C");
  }

  // Update humidity display (original format)
  if (!isnan(hum)) {
    lcd.setCursor(10, 1);
